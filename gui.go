...
// Size returns DPI-aware sizes. On Windows, Fyne handles DPI scaling automatically
// via the OS-level scaling factor. We apply a small padding increase on Windows
// to ensure form elements don't clip text with ClearType rendering.
...
// windowSize returns the default window size adjusted for the platform.
// On Windows, Fyne respects the OS DPI scaling, but we use slightly larger
// base dimensions so form fields are not cramped at 100% scaling on 1080p.
...
func runGUI(cfgPath string, cfg *Config) {
	...
	g.win.Resize(windowSize())
	g.win.SetFixedSize(false) // ensure resizable
	g.buildUI(cfg)
